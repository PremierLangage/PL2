...
export class AutomatonEditorService {
    ...
    private readonly renameStateEvent = new Subject<{ oldName: string, newName: string }>();
    ...
    private get automaton() {
        ...
        if (!automaton.position)
            automaton.position = {};
        if (!automaton.states)
            automaton.states = [];
        if (!automaton.initialStates)
            automaton.initialStates = [];
        if (!automaton.acceptingStates)
    ...
    removeTransition(transition: Transition) {
        if (this.remove(this.transitions, e => {
            return e.fromState === transition.fromState && e.toState === transition.toState;
        })) {
            this.validate();
