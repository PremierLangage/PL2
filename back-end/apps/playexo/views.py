...
def filter_by_grade(minInRequest: bool, maxInRequest: bool, request: HttpRequest, sql_request: Q):
    """
        This function will filter the final result by the grade,
        the grade must be inferior/superior or between
...
def filter_by_date(startInRequest: bool, endInRequest: bool, request: HttpRequest, sql_request: Q):
    """
        This function will filter the final result by the date,
        it must be inferior/superior or between
...
def filter_by_pl(plInRequest: bool, request: HttpRequest, sql_request: Q):
    """
        This function return a new request which will filter
        our final request with a pl in particular
    """
    if plInRequest:
        try:
            try:
...
def filter_by_course(courseInRequest: bool, request: HttpRequest, sql_request: Q, parents: dict):
    if courseInRequest:
        try:
            courseId = int(request.GET["course"])
            activitiesLinkToCourse = filter(
                (lambda activity: find_course(parents, activity[1])[0] == courseId),
