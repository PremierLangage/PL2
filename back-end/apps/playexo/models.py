...
class SessionExerciseAbstract(models.Model):
    ...
    def reroll(self, seed: int, grade: Optional[int]):
        ...
        Seed must be reroll if:
            ...
            - oneshot is set, grade is provided and less than oneshot_threshold
              (100 if not provided)"""
        oneshot: bool = bool(self.get_from_context('settings.oneshot'))
    ...
    def raw_evaluate(self, request: HttpRequest, answers: dict, test: bool=False):
        ...
        """
        evaluator: SandboxEval = SandboxEval(self.envid, answers, None)
        if not evaluator.check():
            self.build(request, test=test)
            evaluator = SandboxEval(self.envid, answers, None)
